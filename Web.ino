...
const char HTTP_CONF[] PROGMEM = "<div><label>Ger&auml;tename: (CUxD-Name)</label></div><div><input type='text' id='devicename' name='devicename' pattern='[A-Za-z0-9_ -]+' placeholder='Ger&auml;tename' value='{dn}'></div><div><label>{st}:</label></div><div><input type='text' id='ccuip' name='ccuip' pattern='((^|\\.)((25[0-5])|(2[0-4]\\d)|(1\\d\\d)|([1-9]?\\d))){4}$' maxlength=16 placeholder='{st}' value='{ccuip}'></div><div><label>Motorumschaltzeit<br>(0.50-25.50 Sek.):</label></div><div><input type='text' id='mst' name='mst' pattern='[0-9.]+' placeholder='Motorumschaltzeit' value='{mst}'></div><div><label>Fahrzeit hoch<br>(0.10-6000.00 Sek):</label></div><div><input type='text' id='up' name='up' pattern='[0-9.]+' placeholder='Fahrzeit hoch' value='{up}'></div><div><label>Fahrzeit runter<br>(0.10-6000.00 Sek):</label></div><div><input type='text' id='down' name='down' pattern='[0-9.]+' placeholder='Fahrzeit hoch' value='{down}'></div><div><label>Fahrten bis zur automatischen<br>Kalibrierfahrt (0-255)</label></div><div><input type='text' id='cal' name='cal' pattern='[0-9]+' placeholder='Kalibrierung nach x Fahrten' value='{cal}'></div>";
